...
def lambda_handler(event, context):
    ...
    if not isinstance(user, int) or user <= 0:
        return {
            'statusCode': 400,
            'body': json.dumps({'message': 'El ID del usuario es un número inválido'})
        }

    if not isinstance(movie, int) or movie <= 0:
        return {
            'statusCode': 400,
            'body': json.dumps({'message': 'El ID de la película es un número inválido'})
        }
...
def insert_into_comments(user, movie, comment):
    ...
    try:
        with connection.cursor() as cursor:
            insert_query = """
                INSERT INTO Comments (user_id, movie_id, comment) VALUES (%s, %s, %s)
            """
